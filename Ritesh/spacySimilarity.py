...
# Load the spaCy medium or large model
...
# Function to extract relevant sentences based on keyword matching
def extract_relevant_sentences(doc):
    relevant_sentences = set()  # Use a set to store unique sentences
    matches = matcher(doc)
    for match_id, start, end in matches:
        span = doc[start:end]
        sentence = span.sent
        relevant_sentences.add(sentence.text)  # Add sentence to set
    return list(relevant_sentences)  # Convert set to list for consistent output order
